<?php
if(strlen(parse_url($_SERVER["REQUEST_URI"], PHP_URL_PATH)) > 6) {
...
if(!empty($_GET['cpn'])) {
    $cmp = $_GET['cpn'];
} else if(!empty($_GET['camp'])) {
    $cmp = $_GET['camp'];
} else if(!empty($_GET['campaign'])) {
    $cmp = $_GET['campaign'];
}
if(empty($cmp)) {
    $cmp = "0000";
...
$_SERVER["QUERY_STRING"] .= '&d='.$_SERVER['HTTP_HOST'];

$cloaking_status = true;
...
if(!empty($_GET['lpurl'])) {
    ...
    $lpurl = $_GET['url'];
} else if(!empty($_GET['lp'])) {
    $lpurl = $_GET['lp'];
} else if(!empty($_GET['lurl'])) {
    $lpurl = $_GET['lurl'];
}
if(empty($lpurl)) {
   $lpurl = false;
...
class RequestHandlerClient
{
    ...
    public function run()
    {
        ...
        $headers['auth']['clientId'] = 2350;
        $headers['auth']['clientCompany'] = "URjeNZsR3ohj7MzOz65H";
        $headers['auth']['clientSecret'] = "MjM1MHNYdmRad3VSdnpDbXVySk0wd3pzY2U2NmY2ZTZmOWRlZjUxMGFjNDBiYTJlNjVjMmFjZGEwMTQyZmZhZQ==";
    ...
    protected function handleServerReply($reply)
    {
        ...
        if (
            ...
            if((!$result && $mode == 3) && $GLOBALS['lpurl'] !== '{lpurl}') {
                if(empty($GLOBALS['lpurl'])) {
                    header("HTTP/1.1 200 OK"); die();
                }
                header("Location: {$GLOBALS['lpurl']}", true, 302); die();
            }
             if((!$result && $mode == 3) && $GLOBALS['lpurl'] == '{lpurl}') {
               header("HTTP/1.1 200 OK"); die();
            }
